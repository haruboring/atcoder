...
signed main() {
    ...
    // max = 800, loop = 3でTLE
    // max = 600, loop = 5でいいや。まあこれ微調整したところで2倍未満の改善だから、これで諦める。→TLE
    // max = 550, loop = 5でやる。→7926125
    int max = 550;
    int loop = 5;
    debug(K);

    if (K < 500) loop = 10;
    // なんかスコア下がったんだが、これ追加したら、
    // ループ増やす方が一般的によくない？何回も試してくれるんだから。
    // わからず。あとで確認する。
    if (K < 100) loop = 30;
