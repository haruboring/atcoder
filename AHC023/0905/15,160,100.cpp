...
signed main() {
    ...
    vector<tuple<int, int, int, int>> D_S_DSK(K);
    for (int k = 0; k < K; ++k) {
        D_S_DSK[k] = {D[k] - S[k], D[k], S[k], k};
        debug(S[k]);
    }
    ...
    vector<tuple<int, int, int>> DSK(min(K, H * W));
    for (int i = 0; i < min(K, H * W); i++) {
        DSK[i] = {get<1>(D_S_DSK[i]), get<2>(D_S_DSK[i]), get<3>(D_S_DSK[i])};
    }
    sort(all(DSK));
    reverse(all(DSK));

    map<int, pair<int, int>> ds_by_k;
    for (int k = 0; k < K; ++k) {
        ds_by_k[k] = {D[k], S[k]};
    }

    int now_planted_cnt = 0;
    int time = 0;
    ...
    multiset<int> st;
    for (int i = 0; i < min(K, H * W); i++) {
        st.insert(get<1>(DSK[i]));
    }

    for (int i = 0; i < min(K, H * W); i++) {
        auto [d, s, k] = DSK[i];
        auto [dis, ii, jj] = dist_i_j[i];
        plan.push_back({k, ii, jj, 1});
        planted.insert(k);
    }
