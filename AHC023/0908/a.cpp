...
signed main() {
    ...
    map<pair<int, int>, vector<pair<int, int>>> set_planted;
    map<pair<int, int>, vector<int>> mp_d;

    // １周目の、最後に入れたもの（出入り口を塞いでいるもの）が2周目以降まで生きることがある。まずい。
    ...
    for (int i = 0; i < K; i++) {
        ...
        if (cnt >= H * W || i == K - 1) {
            ...
            for (int j = 0; j < tmp.size(); j++) {
                ...
                auto [k, dummy_s] = tmp[j];
                if (std == 0) {
                    set_planted[{i1, j1}].push_back({k, dummy_s});
                    mp_d[{i1, j1}].push_back(D[k]);
                    continue;
                }

                vector<int> mpdd = mp_d[{i1, j1}];
                int idx = lower_bound(all(mpdd), S[k]) - mpdd.begin();
                if (idx == mpdd.size()) {
                    set_planted[{i1, j1}].push_back({k, S[k]});
                    mp_d[{i1, j1}].push_back(D[k]);
                    continue;
                }

                auto [prek, pres] = set_planted[{i1, j1}][idx];

                int sum = 0;
                for (int l = idx; l < mpdd.size(); l++) {
                    int k = set_planted[{i1, j1}][l].first;
                    sum += D[k] - S[k];
                }

                if (D[k] - S[k] > sum) {
                    for (int l = idx; l < mpdd.size(); l++) {
                        set_planted[{i1, j1}].pop_back();
                        mp_d[{i1, j1}].pop_back();
                    }
                    set_planted[{i1, j1}].push_back({k, min(S[k], pres)});
                    mp_d[{i1, j1}].push_back(D[k]);
                }
            }
            tmp.clear();
    ...
    vector<vector<tuple<int, int, int>>> tmp_plan(100, vector<tuple<int, int, int>>(0));
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            int idx = 0;
            for (auto [k, s] : set_planted[{i, j}]) {
                tmp_plan[idx].push_back({D[k], k, s});
                idx++;
            }
        }
    }

    vector<tuple<int, int, int, int>> plan(0);
    rep(i, 100) {
        if (tmp_plan[i].size() == 0) continue;
        sort(all(tmp_plan[i]));
        reverse(all(tmp_plan[i]));
        int idx = 0;
        for (auto [dk, k, s] : tmp_plan[i]) {
